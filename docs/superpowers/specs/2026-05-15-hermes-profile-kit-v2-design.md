...
## Amendment — 2026-05-15: upstream uses argparse, not Click

Verified that `hermes_cli/main.py` builds its CLI with `argparse.ArgumentParser` + `subparsers.add_parser(...)`, not Click. There is no importable `cli` Click group. All references to "Click tree", "Click walker", and `click_walker.py` in sections 0, 1, 5, and 6 of this spec should be read as:

- "Click tree" → "argparse tree" (the tree of `subparsers.add_parser(...)` invocations inside `hermes_cli/main.py`)
- "walking the Click tree" → "AST-walking `hermes_cli/main.py` to enumerate `subparsers.add_parser(...)` calls"
- `click_walker.py` → `argparse_walker.py`
- "import `hermes_cli.main` and obtain the Click root" → "read `hermes_cli/main.py` as a file path; do not import; AST-parse"

The output shape (`{path, params, help, hidden}`) and the consumers (`cmd_index.py`, `validate.py`, `regen_docs.py`) are unchanged. `hpk`'s own CLI continues to use Click — only the upstream walker changes.
...
- AST-walks Hermes' `hermes_cli/main.py` argparse tree at CI time (see Amendment 2026-05-15) to auto-generate `docs/commands.md` and validate every Hermes command the kit references.
...
.github/workflows/upstream-sync.yml  (daily cron)
  ...
  2. AST-walk hermes_cli/main.py argparse tree → build/cmd_index.json
...
## Verified hermes commands (extracted from upstream argparse tree)
...
...
│   │   └── exa.py
│   ├── codegen/
│   │   ├── argparse_walker.py        # CI-time only (AST-parses hermes_cli/main.py)
│   │   ├── cmd_index.py
│   │   └── validate.py
│   └── ui.py                         # rich console
├── scripts/
...
Unit
  manifest models, token validators, argparse_walker on toy fixture, wizard with patched questionary
```
